...
from main import build_optimizer_and_scheduler, compute_context_scores
...
def run_one(seed):
    cfg = Config()
    cfg.data_path = r"C:\Users\ASUS\Desktop\data\Amazon.mat"
    cfg.seed = int(seed)
    cfg.score_mode = "recon_cstd"
    cfg.recon_cstd_weight = 0.7692308
    cfg.use_structural_fusion = False
    cfg.enable_aux_fusion = True
    cfg.aux_score_type = "if_h"
    cfg.aux_fusion_weight = 0.35
    cfg.aux_if_estimators = 500
    ...
    for _ in range(cfg.epochs):
        ...
        preds_a, preds_b, z_views = model(
            x_view_a, [edge_a_v1, edge_a_v2], x_view_b, batch_edges
        )
    ...
    with torch.no_grad():
        ...
        context_std_scores = minmax_np(context_std_scores)
        base = cfg.recon_cstd_weight * recon_scores + (1.0 - cfg.recon_cstd_weight) * context_std_scores
        base = minmax_np(base)
    ...
    iso.fit(h_np)
    if_h = minmax_np(-iso.score_samples(h_np))
    final_scores = (1.0 - cfg.aux_fusion_weight) * base + cfg.aux_fusion_weight * if_h
...
def main():
    ...
    for s in seeds:
        ...
        aucs.append(auc)
        aps.append(ap)
        print(f"seed={s} AUC={auc:.4f} AP={ap:.4f}", flush=True)
